...
class Assets {
    static profileUpload = (req, res, next) => {
        ...
        if (file) {
            if (!file.avatar) {
                res.status(400).json(ApiModel.getApiModel(Enum.status.FAILED, "Field name must be avatar"));
            } else {
                if (!checkExtension(file.avatar)) {
                    res.status(400).json(ApiModel.getApiModel(Enum.status.FAILED, "Invalid Extension!!! Only .png, .jpg, .jpeg and .svg format is required"));
                } else {
                        ...
                        if (req.user?.avatar?.picture) {
                            ...
                            if (existsSync(unlinkFile)) {
                                unlink(unlinkFile, (err) => {
                                    if (err) res.status(500).json(ApiModel.getApiModel(Enum.status.FAILED, "Something went wrong"));
                                });
                            }
            ...
            res.status(404).json(ApiModel.getApiModel(Enum.status.FAILED, "Image is missing"));
        }
    ...
    static fileUpload = (req, res, next) => {
        ...
        if (file) {
            if (!file.image) {
                res.status(400).json(ApiModel.getApiModel(Enum.status.FAILED, "Field name must be image"));
            } else {
                if (!checkExtension(file.image)) {
                    res.status(400).json(ApiModel.getApiModel(Enum.status.FAILED, "Invalid Extension!!! Only .png, .jpg, .jpeg format is required"));
                } else {
                    if (file.image.size < MAXSIZE) {
                        ...
                        file.image.mv(`${LOC}`, async err => {
                            if (err) {
                                ...
                                asset.save();
                                res.status(200).json(ApiModel.getApiModel(Enum.status.SUCCESS, `${filename} image uploaded successfully`))
                            }
                        });
                    } else {
    ...
    static getFile = (req, res) => {
        ...
        if (filename) {
            ...
            res.download(LOC, function (error) {
                if (error)
                    res.status(404).json(ApiModel.getApiModel(Enum.status.FAILED, "Image not found"));
            })
        } else {
            console.log(filename)
            res.status(404).json(ApiModel.getApiModel(Enum.status.FAILED, "Image not found"));
    ...
    static getImage = async (req, res) => {
        ...
        if (imageURI) {
            ...
            if (imageList.length > 0) {
                ...
                res.json(ApiModel.getApiModel(Enum.status.SUCCESS, "Image found", { totalImage: imageList.length ,"images": newImageList }));
            } else {
                res.status(404).json(ApiModel.getApiModel(Enum.status.FAILED, "Nothing to show"))
            }
        }
    }
}
