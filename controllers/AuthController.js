...
class AuthController {
  ...
  static createUserWithEmailAndPassword = async (req, res) => {
    ...
    const { name, email, password, password_confirmation, tc, username } =
      req.body;
    if (name && email && password && password_confirmation && username) {
      if (!Utils.ValidateEmail(email)) {
        ...
        const { isStrong, response } = await Utils.checkPasswordValidation(
          password
        );
        if (!isStrong) {
          res
            .status(401)
            .json(ApiModel.getApiModel(Enum.status.FAILED, response));
        } else {
          try {
            ...
            if (err.code == 'auth/email-already-in-use') {
              ...
              res
                .status(400)
                .json(ApiModel.getApiModel(Enum.status.FAILED, err.message));
            }
          }
        }
      }
  ...
  static signInWithEmailAndPassword = async (req, res) => {
    ...
    if (!email || !password) {
      ...
      try {
            ...
            await signInWithEmailAndPassword(getAuth(), email, password)
              ...
              .catch((e) => {
                if (e.code == 'auth/user-not-found')
                  ...
                  res
                    .status(500)
                    .json(ApiModel.getApiModel(Enum.status.FAILED, e.message));
              });
          }
        }
      } catch (error) {
        res
          .status(500)
          .json(ApiModel.getApiModel(Enum.status.FAILED, error?.message));
      }
    }
  };
    ...
    if (password && password_confirmation) {
      if (password !== password_confirmation) {
        ...
        if (!isStrong) {
          ...
          try {
            let isOldAndNewPasswordMatch = await Utils.matchPassword(
              password,
              req.user.password
            );
      ...
      if (!email) {
        ...
        if (!user) {
          res
            .status(404)
            .json(
              ApiModel.getApiModel(Enum.status.FAILED, "User doesn't exists")
            );
        } else {
