...
const fetchData = async () => await StreamModel.find()
...
class StreamController {
    ...
    static addStream = async (req, res) => {
        try {
            ...
            if (!stream) {
                ...
                if (!title) {
                    sendResponse(res, 400, Enum.status.FAILED, "Title is required");
                    return;
                }
                ...
                const isSteamExist = data.find(
                    (f) => f.title.toLowerCase() === title?.toLowerCase()
                );

                console.log("=======================")
                // console.log(isSteamExist);
                if (isSteamExist) {
                    sendResponse(res, 401, Enum.status.FAILED, `${title} already exists`);
                    return;
                }

                // console.log("isExistStream", isExistStream);
                if (isExistStream) {
                    await res
                        .status(403)
                        .json(
    ...
    static getStream = async (req, res) => {
        ...
        try {
            ...
            if (!streamData) {
                sendResponse(res, 404, Enum.status.NOTFOUND, `${stream} stream does not exist`);
                return;
            }

            if (!branch) {
                sendResponse(res, 200, Enum.status.SUCCESS, `There is ${stream} stream`, streamData);
                return;
            }
            ...
            if (!branchData) {
                sendResponse(res, 404, Enum.status.NOTFOUND, `${branch} branch does not exist`);
                return;
            }

            if (!subject) {
                sendResponse(res, 200, Enum.status.SUCCESS, `There is ${branch} branch`, branchData);
                return;
            }
            ...
            if (!subjectData) {
                sendResponse(res, 404, Enum.status.NOTFOUND, `${subject} subject does not exist`);
                return;
            }

            if (!chapter) {
                sendResponse(res, 200, Enum.status.SUCCESS, `There is ${subject} subject`, subjectData);
                return;
            }
            ...
            if (!chapterData) {
                sendResponse(res, 404, Enum.status.NOTFOUND, `${chapter} chapter does not exist`);
                return;
            }

            sendResponse(res, 200, Enum.status.SUCCESS, `There is ${chapter} chapter`, chapterData);
